...
class TestViews(base.UserTestCase):
    ...
    def test_private_mixin_detail_view_is_accessible_by_owner(self):
    ...
    def test_private_mixin_detail_view_is_not_accessible_by_anonymous(self):
        m = G(test_permission.models.PrivateReadModel, owner=self.users[0], read_private=True)

        response = self.client.get(m.reverse('detail'))
        self.assertEqual(response.status_code, 404)

    def test_private_mixin_detail_view_is_accessible_by_authorized_user(self):
        m = G(test_permission.models.PrivateReadModel, owner=self.users[0], read_private=True)
